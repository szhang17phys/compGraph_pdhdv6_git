...
#===test single Optical Channel==================================
    ...
    cut_z = (pos[:,2] > -300) & (pos[:,2] < 600)
    cut_y = (pos[:,1] > -450) & (pos[:,1] < 450)
...
#---Change label each time--------------------------------------
    label = 14 #opCh label---
    opName = 'opch14'#change name each time---
#---------------------------------------------------------------
...
#Keep data of certain opCh and keep as txt files=================
    ...
    print("Total points (Length of bias Values): ", len(hist))
    print("visib_diff: ", hist)

    for nums in range(0, len(hist)): 
        pos_X.append(pos[nums, 0])
        pos_Y.append(pos[nums, 1])
        pos_Z.append(pos[nums, 2])
        value_bias.append(hist[nums])
        value_true.append(true_x_s[nums, label]*1000000)
        value_emul.append(emul_x_s[nums, label]*1000000)

    print("size of value_simu: ", len(value_true))
    print("size of value_emul: ", len(value_emul))

   #keep x, y, z & values in  txt files---
    with open('{}_xPos.txt'.format(opName), 'w') as filehandle:
        for listitem in pos_X:
            filehandle.write('%f\n' % listitem)

    with open('{}_yPos.txt'.format(opName), 'w') as filehandle:
        for listitem in pos_Y:
            filehandle.write('%f\n' % listitem)

    with open('{}_zPos.txt'.format(opName), 'w') as filehandle:
        for listitem in pos_Z:
            filehandle.write('%f\n' % listitem)

    with open('{}_biasValues.txt'.format(opName), 'w') as filehandle:
        for listitem in value_bias:
            filehandle.write('%f\n' % listitem)

    with open('{}_simuValues.txt'.format(opName), 'w') as filehandle:
        for listitem in value_true:
            filehandle.write('%d\n' % listitem)

    with open('{}_emulValues.txt'.format(opName), 'w') as filehandle:
...
#This is old and unmature codes, need further modifications---
